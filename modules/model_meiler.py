...
class AtomEncoder(torch.nn.Module):
    def __init__(self, emb_dim, feature_dims, sigma_embed_dim, lm_embedding_type= None):
        ...
        if self.lm_embedding_type is not None:
            ...
            self.lm_embedding_layer = torch.nn.Linear(self.lm_embedding_dim + emb_dim + 64 , emb_dim)

    def forward(self, x_input):
        x_embedding = 0
        # x_input = x_input.to("cuda:0")
        res_fea = x_input[:, :1281]
        meiler_feature_0 = x_input[:, 1281: 1288]  # (L*128)
        meiler_feature = self.meiler_net(meiler_feature_0)
        other_fea = x_input[:, 1288:]
        x = torch.cat((res_fea, other_fea), dim=1)
        if self.lm_embedding_type is not None:
        ...
        if self.lm_embedding_type is not None:
            # x_embedding = self.lm_embedding_layer(torch.cat([x_embedding, x[:, -self.lm_embedding_dim:], meiler_feature], dim=1))  # l*16
            x_embedding = self.lm_embedding_layer(torch.cat([x_embedding, x[:, 1:1281], meiler_feature], dim=1))  # l*16
        return x_embedding
...
class ScoreModel(torch.nn.Module):
    def __init__(self, t_to_sigma, device, timestep_emb_func, sigma_embed_dim=32, sh_lmax=2,
                 ns=23, nv=4, num_conv_layers=2, rec_max_radius=15, c_alpha_max_neighbors=24,
