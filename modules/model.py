...
class AtomEncoder(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.lm_embedding_type is not None:
            ...
            x_embedding = self.lm_embedding_layer(torch.cat([x_embedding, x[:, 1:1281]], dim=1))
        return x_embedding
    ...
    def forward(self, data, eps=1e-12):
        ...
        for l in range(len(self.rec_conv_layers)):  # 2层
            ...
            if l == 0:
                ...
                rec_node_attr = rec_node_attr
                n_norm_vec = n_norm_vec
                c_norm_vec = c_norm_vec
                rec_edge_index = rec_edge_index
                rec_edge_attr_ = rec_edge_attr_
                rec_edge_sh = rec_edge_sh
        ...
        # fix the magnitude of translational and rotatsional score vectors
        res_tr_norm = torch.linalg.vector_norm(res_tr_pred, dim=1).unsqueeze(1)
        res_tr_pred = res_tr_pred
        res_tr_norm = res_tr_norm
        res_tr_norm = res_tr_norm
        data.graph_sigma_emb = data.graph_sigma_emb
