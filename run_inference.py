...
parser.add_argument('--model_dir', type=str, default='/mydata/cuixinyue/multi_conformation/multi_version_DiGData_sasa_state1/save_model_test', help='Path to folder with trained score model and hyperparameters')
...
parser.add_argument('--cache_path', type=str, default='/node1data/cuixinyue/cacheonly_PDB91APO', help='Folder from where to load/restore cached dataset')
...
for i in range(len(data_list)):
    path = os.path.join(args.protein_path, f"input/{data_list[i]}.pdb")
...
def predict_one_complex(orig_structure_graph, model, res_tr_schedule, res_rot_schedule, res_chi_schedule, t_to_sigma, N, args, device, ):
    ...
    now_pdbpath = os.path.join(args.protein_path, f"input/{orig_structure_graph['name'][0]}.pdb")
    ...
    write_dir = f"{args.out_dir}/output"
...
for idx, orig_structure_graph in enumerate(test_loader):
