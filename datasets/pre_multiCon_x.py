...
from datasets.process_x import get_rec_graph, extract_structure, parse_structures
...
class Pre_multiConf(Dataset):
    ...
    def get_structure(self, par):
        ...
        try:
            ...
            get_rec_graph(name, rec, state1, rec_coords, c_alpha_coords, n_coords, c_coords, chis, chi_masks, structure_graph, rec_radius=self.structure_radius,
                       c_alpha_max_neighbors=self.c_alpha_max_neighbors, all_atoms=self.all_atoms,
                         atom_radius=self.atom_radius, atom_max_neighbors=self.atom_max_neighbors, remove_hs=self.remove_hs, lm_embeddings=lm_embeddings,profile_path=self.profile_features_path)
        except Exception as e:
            print(f'Skipping 1 {name} because of the error:')
            print(e)
        ...
        try:
            ...
            print(f'Skipping 2 {name} because of the error:')
            print(e)
